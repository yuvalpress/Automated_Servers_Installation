...
class iDracConf:
    ...
    def setName(self):
        if self.name != "N/A" and "." not in self.name:
            try:
                sub = subprocess.Popen(["powershell",
                                        "& racadm -r {} -u root -p password set iDRAC.Nic.DNSRacName iDrac-{}".format(
                                            self.ip, self.name)], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
                output = sub.stdout.readlines()
                for line in output:
                    if "successfully" in str(line) or "Success" in str(line):
                        print(
                            colors.OKGREEN + "Successfully changed iDrac's name to iDrac-{} at ip {}.".format(self.name,
                                                                                                              self.ip) + colors.ENDC)
    ...
    def setTZ(self):
        if self.timezone != "N/A":
            try:
                sub = subprocess.Popen(["powershell",
                                        "& racadm -r {} -u root -p password set idrac.time.timezone {}".format(
                                            self.ip, self.timezone)], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
                output = sub.stdout.readlines()
                for line in output:
                    if "successfully" in str(line) or "Success" in str(line):
                        print(colors.OKGREEN + "Successfully changed iDrac's Timezone to {} for server {}.".format(
                            self.timezone, self.name) + colors.ENDC)
    ...
    def setBootMode(self):
        if self.boot_mode != "N/A":
            try:
                sub = subprocess.Popen(["powershell",
                                        "& racadm -r {} -u root -p password set BIOS.BiosBootSettings.BootMode {}".format(
                                            self.ip, self.boot_mode)], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
                output = sub.stdout.readlines()
                for line in output:
                    if "successfully" in str(line) or "Success" in str(line):
                        print(colors.OKGREEN + "Successfully changed iDrac's Boot Mode to {} for server {}.".format(
                            self.boot_mode, self.name) + colors.ENDC)
    ...
    def setVconsole(self):
        if self.vconsole != "N/A":
            try:
                sub = subprocess.Popen(["powershell",
                                        "& racadm -r {} -u root -p password set idrac.VirtualConsole.plugintype {}".format(
                                            self.ip, self.vconsole)], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
                output = sub.stdout.readlines()
                for line in output:
                    if "successfully" in str(line) or "Success" in str(line):
                        print(
                            colors.OKGREEN + "Successfully changed iDrac's Virtual Console to {} for server {}.".format(
                                self.vconsole, self.name) + colors.ENDC)
    ...
    async def iDrac_Raids(self):
        ...
        print("Raid Level: ", raidLevel)
        print("Raid Names: ", raidNames)
        print("Disks: ", disks)
        ...
        if self.raid1_check == 1:
            try:
                ...
                print(
                    colors.OKPURPLE + "Configuring VD_0 ({}) for server {}\nThe raid will be created on disks: {}".format(
                        raidNames[0], self.name, bayString) + colors.ENDC)
                sub = subprocess.Popen(["powershell",
                                        "& racadm -r {} -u root -p password raid createvd:{} -rl {} -wp wb -rp ra -pdkey:{} -name {}".format(
                                            self.ip, controller, raidLevel[0], diskString, raidNames[0])],
                                       stdout=subprocess.PIPE, stderr=subprocess.PIPE)

                print("stdout: ", sub.stdout.read())
                ...
                if len(sub.stderr.readlines()) == 0:
                    sub = subprocess.Popen(["powershell",
                                            "& racadm -r {} -u root -p password --nocertwarn jobqueue create {} --realtime".format(
                                                self.ip, controller)], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
                    out = sub.stdout.readlines()
                    print("Error: ", out)
                    output = str(out[2]).strip("b'").replace("\\r\\n", "").replace(" ", "").replace("\\r", "")
                    JID = output.split("=")[1]
                        ...
                        while trueUntil:
                            ...
                            sub = subprocess.Popen(["powershell",
                                                    "& racadm -r {} -u root -p password --nocertwarn jobqueue view -i {}".format(
                                                        self.ip, JID)], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
                            ...
                            if "100" in line7 and "Completed" in line3:
                                print(colors.OKGREEN + "The creation of {} succeeded for server: {} ({})".format(
                                    raidNames[0], self.name, self.ip) + colors.ENDC)
                                print(colors.OKGREEN + "Job execution finished successfully for server: {} ({})".format(
                                    self.name, self.ip) + colors.ENDC)
                                trueUntil = False

                            elif "100" in line7 and "Failed" in line3:
                                print(colors.FAIL + "The creation of {} on server {} ({}) have failed".format(
                                    raidNames[0], self.name, self.ip) + colors.ENDC)
                                print(colors.FAIL + "Job DONE for server: {} ({})".format(self.name,
                                                                                          self.ip) + colors.ENDC)
                                trueUntil = False

            except Exception as err:
                print(
                    colors.FAIL + "The creation of {} on server {} ({}) have failed with the following error: {}".format(
                        raidNames[0], self.name, self.ip, err) + colors.ENDC)
        ...
        if self.raid2_check == 1:
            ...
            try:
                print(
                    colors.OKPURPLE + "Configuring VD_1 ({}) for server {}\nThe raid will be created on disks: {}".format(
                        raidNames[0], self.name, bayString) + colors.ENDC)
                sub = subprocess.Popen(["powershell",
                                        "& racadm -r {} -u root -p password raid createvd:{} -rl {} -wp wb -rp ra -pdkey:{} -name {}".format(
                                            self.ip, controller, raidLevel[1], diskString, raidNames[1])],
                                       stdout=subprocess.PIPE, stderr=subprocess.PIPE)
                ...
                if len(sub.stderr.readlines()) == 0:
                    ...
                    if len(sub.stderr.readlines()) == 0:
                        ...
                        while trueUntil:
                            ...
                            print("{} Raid creation progress: ".format(self.name), line7)
                            if "100" in line7 and "Completed" in line3:
                                print(colors.OKGREEN + "Succeeded configure {} for server: {}".format(raidNames[1],
                                ...
                                print(colors.FAIL + "Failed to configure {} for server: {}".format(raidNames[1],
                                                                                                   self.name) + colors.ENDC)
                                print(colors.FAIL + "Job DONE for server: {}".format(self.name) + colors.ENDC)
                                trueUntil = False

            except Exception as err:
                print(colors.FAIL + "The creation of {} on server {} have failed with the following error: {}".format(
                    raidNames[1], self.name, err) + colors.ENDC)
    ...
    def iDrac_IP(self):
        if self.ip_check == 1:
            if ping(self.tmp_ip):
                try:
                    sub = subprocess.Popen(["powershell",
                                            "& racadm -r {} -u root -p password Setniccfg -s {} {} {}".format(
                                                self.tmp_ip, self.ip, self.subnet, self.gateway)],
                                           stdout=subprocess.PIPE, stderr=subprocess.PIPE)
                    output = sub.stdout.readlines()
                    for line in output:
                        if "successfully" in str(line) or "Success" in str(line):
                            print(
                                colors.OKGREEN + "Successfully changed iDrac's IP Address to {}, {}, {} for server {}.".format(
                                    self.ip, self.subnet, self.gateway, self.name) + colors.ENDC)

                except Exception as err:
                    print(err)

    def set_Worker_pdisks(self):
        if ping(self.ip):
            controller = raidData(self.ip).getControllers()
            ...
            # collect all disks in "non-raid" state
            all_ready = []
            for disk, state in zip(range(0, len(output), +2), range(1, len(output), +2)):
                if "Online" not in str(output[state]) and "Ready" in str(output[state]):
                    all_ready.append(str(output[disk]).replace(" ", "").replace("\\r\\n", "").strip("b'"))
                    sub = subprocess.Popen(["powershell",
                                            "& racadm -r {} -u root -p password --nocertwarn storage converttononraid:{}".format(
                                                self.ip,
            ...
            if len(all_ready) != 0:
                sub = subprocess.Popen(["powershell",
                                        "& racadm -r {} -u root -p password --nocertwarn jobqueue create {} --realtime".format(
                                            self.ip, controller)], stdout=subprocess.PIPE,
                                       stderr=subprocess.PIPE)
                output = sub.stdout.readlines()
                line = str(output[2]).strip("b'").replace("\\r\\n", "").replace(" ", "").replace("\\r", "")

                JID = line.split("=")[1]
                print("Job has been initiated successfully on server {}".format(self.ip))
                ...
                if len(sub.stderr.readlines()) == 0:
                    ...
                    while trueUntil:
                        ...
                        sub = subprocess.Popen(["powershell",
                                                "& racadm -r {} -u root -p password --nocertwarn jobqueue view -i {}".format(
                                                    self.ip, JID)], stdout=subprocess.PIPE,
                                               stderr=subprocess.PIPE)
                        lines = sub.stdout.readlines()
                        line3 = str(lines[3]).strip("b'").replace("\\r\\n", "").replace(" ", "").replace("\\r", "")
                        line7 = str(lines[7]).strip("b'").replace("\\r\\n", "").replace(" ", "").replace("\\r", "")
        ...
        for i in range(6, sheet.nrows, +1):
            if "1" in str(sheet.cell_value(i, 1)):
                if "Kafka" in sheet.cell_value(i, 4) or "Worker" in sheet.cell_value(i, 4):
                    if ping(sheet.cell_value(i, 8)) or ping(sheet.cell_value(i, 9)):
                        sub = subprocess.Popen(
                            ["powershell", "& racadm -r {} -u root -p password storage get vdisks".format(sheet.cell_value(i, 8))],
                            stdout=subprocess.PIPE, stderr=subprocess.PIPE)
                        output = sub.stdout.readlines()
                        for line in output:
...
class raidData:
    ...
    def getDisksNames(self):
        ...
        try:
            sub = subprocess.Popen(["powershell",
                                    "& racadm -r {} -u root -p password --nocertwarn storage get pdisks -o -p size".format(
                                        self.ip)], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            output = sub.stdout.readlines()
            pdisks = {}
            for disk, size in zip(range(0, len(output), +2), range(1, len(output), +2)):
                pdisks[str(output[disk]).replace(" ", "").replace("\\r", "").strip("b'").strip("\\n")] = str(
                    output[size]).replace(" ", "").replace("\\r\\n", "").strip("b'")
                ...
                if self.raid1_check == 1:
                    ...
                    if type(self.disks[0]) is not str:
                        ...
                        for disk, sizeCount in pdisks.items():
                            size = str(sizeCount).split("=")[1].replace("GB", "")
                            if str(sortedSizes[0]) in size:
                                cnt += 1
...
if __name__ == "__main__":
    ...
    for server in excelData:
        try:
            # Create iDrac object
            iDrac = iDracConf(server["name"], server["tmp_ip"], server["ip_address"], server["subnet"],
                              server["gateway"],
                              server["timezone"], server["vconsole"], server["boot_mode"], server["vdisks"],
                              server["pdisks"], server["IP_Check"], server["Raid1_Check"], server["Raid2_Check"])
    ...
    for server in excelData:
        print("====================Starting iDrac configuration process for server {} ({})====================".format(
        ...
        try:
            ...
            while ping(server["ip_address"]) == False:
                print("Waiting for ip address {} of server {} to be reachable".format(server["ip_address"],
                                                                                      server["name"]))
            ...
            print("===================================================================================================")
            print("iDrac configuration process for server {} ({}) failed with the following error: ".format(
                server["name"], server["ip_address"]), err)
            continue
